...
class TestTransportCreation:
    ...
    def test_create_stdio_transport(self):
        """stdio transport 생성"""
        config = {"transport": "stdio", "command": "python", "args": ["server.py"], "env": {"KEY": "value"}}

        with patch("mcp.StdioServerParameters") as mock_params:
            transport = create_transport(config)

            assert isinstance(transport, StdioTransport)
            mock_params.assert_called_once_with(command="python", args=["server.py"], env={"KEY": "value"})
...
class TestMCPClientWithTransport:
    ...
    async def test_client_with_config(self):
        """설정 기반 클라이언트 생성"""
        config = {"transport": "streamable_http", "url": "http://localhost:3000/mcp"}
