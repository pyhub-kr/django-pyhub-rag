...
from pyhub.llm.tools import FunctionToolAdapter, ToolAdapter, create_tool_from_function
...
def test_multiple_tools_function_calling(provider_info):
    ...
    # 복수 도구 테스트
    result = llm.ask("서울 날씨를 알려주고, 5 + 3을 계산해줘", tools=[get_weather, add_numbers], model=model)
...
async def test_async_function_calling(provider_info):
    ...
    result = await llm.ask_async(
        "부산 날씨를 확인하고, 7 더하기 3을 계산해줘", tools=[async_weather_lookup, sync_calculation], model=model
    )
...
def test_error_handling(provider_info):
    ...
    # 1. 정상 작동 테스트
    result1 = llm.ask("값 50을 처리해줘", tools=[error_prone_function, safe_function], model=model)
    ...
    if provider_name == "Upstage":
        ...
        assert (
            "50" in result1.text
            or "처리" in result1.text
            or "error_prone_function" in result1.text
            or "safe_function" in result1.text
        )
        ...
        assert (
            ...
            or "처리" in result1.text
            or "error_prone_function" in result1.text
            or "함수" in result1.text
            or "결과" in result1.text
        )
    else:
        # 다른 제공자들은 실제 결과 포함
    ...
    if provider_name == "Upstage":
        ...
        assert (
            "안전" in result2.text
            or "처리" in result2.text
            or "오류" in result2.text
            or "safe_function" in result2.text
            or "error_prone_function" in result2.text
        )
    elif provider_name == "Google":
        # Google은 때때로 명확화 질문을 할 수 있음
        assert (
            ...
            or "처리" in result2.text
            or "오류" in result2.text
            or "함수" in result2.text
            or "결과" in result2.text
        )
    else:
        # 다른 제공자들은 실제 결과 포함
...
def test_function_calling_performance(provider_info):
    ...
    result = llm.ask("25 곱하기 4를 계산해줘", tools=[simple_calc], model=model)
    end_time = time.time()
    ...
    if provider_name == "Google":
        ...
        assert (
            "100" in result.text
            or "25" in result.text
            or "4" in result.text
            or "곱하기" in result.text
            or "계산" in result.text
        )
    else:
        # 다른 제공자들은 실제 결과 포함
