...
from pyhub.llm.agents import Tool
...
class TestReactAgent:
    ...
    def test_agent_initialization(self, mock_llm, calculator_tool):
        """Agent 초기화 테스트"""
        agent = ReactAgent(llm=mock_llm, tools=[calculator_tool], max_iterations=5)
...
class TestAsyncReactAgent:
    ...
    async def test_async_tool_execution(self):
        ...
        search_tool = Tool(name="search", description="Search the web", func=async_search)
...
class TestCreateReactAgent:
    ...
    def test_create_sync_agent(self):
        """동기 Agent 생성 테스트"""
        mock_llm = Mock()
        calc_tool = Tool(name="calc", description="Calculator", func=lambda x: str(eval(x)))
    ...
    def test_create_async_agent(self):
        ...
        async_tool = Tool(name="async_tool", description="Async tool", func=async_func)
    ...
    def test_create_agent_with_options(self):
        ...
        agent = create_react_agent(mock_llm, [tool], max_iterations=20, timeout=30)
