...
from pyhub.llm.agents.base import AsyncBaseTool, Tool, ValidationLevel
...
class TestTool:
    ...
    def test_tool_creation(self):
        ...
        tool = Tool(name="multiplier", description="Multiply by 2", func=sample_func)
    ...
    def test_async_tool_detection(self):
        ...
        tool = Tool(name="async_multiplier", description="Async multiply by 2", func=async_func)
...
class TestCalculator:
    ...
    def test_calculator_input_validation(self):
        """계산기 입력 검증 테스트"""
        # 유효한 수식
        valid_expressions = ["2 + 2", "10 * (5 + 3)", "3.14 * 2", "(1 + 2) * (3 + 4)", "100 / 5 - 10"]
    ...
    def test_calculator_with_tool_validation(self):
        ...
        # Tool 객체로 변환
        tool = Tool(name=calc.name, description=calc.description, func=calc.run, args_schema=calc.args_schema)
...
class TestAsyncTools:
    ...
    async def test_async_tool_execution(self):
        ...
        class AsyncCounter(AsyncBaseTool):
            def __init__(self):
                self.count = 0
                super().__init__(name="async_counter", description="Async counter")
    ...
    async def test_tool_executor(self):
        ...
        # 동기 도구
        sync_tool = Tool(name="sync", description="Sync tool", func=lambda x: f"sync: {x}")
        ...
        async_tool = Tool(name="async", description="Async tool", func=async_func)
...
class TestToolWithRealWorldScenarios:
    ...
    def test_file_operation_security(self):
        ...
        # 안전한 경로
        safe_paths = ["data.txt", "./logs/app.log", "output/result.json"]

        for path in safe_paths:
            input_model = FileOperationInput(path=path, operation="read")
    ...
    def test_web_search_validation(self):
        ...
        # 유효한 검색
        valid_search = WebSearchInput(query="Python tutorial", max_results=10)
